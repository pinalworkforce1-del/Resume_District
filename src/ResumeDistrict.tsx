...
export default function ResumeDistrict(){
  ...
  const scene=j.scene;
  useEffect(()=>{const updateOnline=()=>setOnline(navigator.onLine);window.addEventListener("online",updateOnline);window.addEventListener("offline",updateOnline);return()=>{window.removeEventListener("online",updateOnline);window.removeEventListener("offline",updateOnline)}},[]);
  ...
  return <main className={`app ${j.largeText?"large-text":""} ${j.reducedMotion?"reduce-motion":""}`}>
    <header className="topbar">
      ...
      <div className="controls">
        ...
        <button className="resume-library" onClick={()=>online&&window.open(MY_RESUMES,"_blank","noopener")} disabled={!online} aria-label={online?"Open My Résumés":"My Résumés requires an internet connection"} title={online?"Open My Résumés":"Available when you are back online"}>{online?"My Résumés":"My Résumés • Online"}</button>
    ...
    <section className="stage" aria-label={titles[scene]}>
      ...
      {scene===12&&resumeComplete&&<div className="completion-actions"><h2>Reflection Complete ✓</h2><p>Put the evidence to work in Resume Builder. Your Level Up account will keep your working résumé and saved targeted versions.</p><div>{online?<a className="primary-action" href={RESUME_BUILDER}>Open Resume Builder →</a>:<button className="primary-action" type="button" disabled title="Available when you are back online">Resume Builder • Online connection required</button>}{online?<a href={MY_RESUMES}>My Résumés</a>:<button type="button" disabled title="Available when you are back online">My Résumés • Online connection required</button>}<button onClick={()=>setModal("review")}>View Resume District Review</button><button onClick={()=>update({scene:1})}>Return to District Map</button><a href={PORTAL}>Enter Interview Arena</a></div></div>}
