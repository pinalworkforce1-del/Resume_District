...
export default function ResumeDistrict(){
  ...
  useEffect(()=>{if(!session||!supabase)return;supabase.from("module_progress").select("journey_state").eq("user_id",session.user.id).eq("module_id",MODULE_ID).maybeSingle().then(({data,error})=>{if(error){setSync("error");return}if(data?.journey_state){const remote={...EMPTY,...data.journey_state} as Journey;setJ(local=>remote.completed.length>local.completed.length||remote.xp>local.xp?remote:local)}setSync("saved")})},[session?.user.id]);
  useEffect(()=>{if(!session||!supabase||!ready)return;setSync("saving");const timer=setTimeout(async()=>{const done=j.completed.includes("reflection");const {error}=await supabase!.from("module_progress").upsert({user_id:session.user.id,module_id:MODULE_ID,journey_state:j,xp:j.xp,is_complete:done,completed_at:j.completionDate||null},{onConflict:"user_id,module_id"});if(!error&&j.name)await supabase!.from("profiles").update({display_name:j.name}).eq("user_id",session.user.id);setSync(error?"error":"saved")},800);return()=>clearTimeout(timer)},[j,session?.user.id,ready]);
